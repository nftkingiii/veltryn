...
import { AlertTriangle, ArrowDownRight, ArrowUpRight, BookOpen, CircleHelp, Clock3, Copy, Database, LineChart, RotateCcw, Save, ShieldCheck, Sparkles, X } from '../ui/icons'
...
export function App() {
  ...
  useEffect(() => {
    ...
    Promise.all([listRwaInstruments(), getTicker(selected), getCandles(selected)])
      .then(([items, live, history]) => { if (!cancelled) { const supported = items.filter((item) => ['NVDAUSDT', 'TSLAUSDT', 'AAPLUSDT'].includes(item.symbol)); setInstruments(supported.length ? supported : items.slice(0, 3)); setTicker(live.ticker); setCandles(history.candles); setProviderState('live') } })
      .catch(() => { if (!cancelled) setProviderState('fallback') })
    ...
    <main id="top">
      {tab === 'rehearse' && <>
        ...
        <section className="workspace-grid">
          <aside className="plan-panel panel"><div className="panel-heading"><div><span className="step-label">01 / POSITION</span><h2>State the trade</h2></div><button className="text-button" onClick={() => { setQuantity(0.1); setCollateral(1000); setLossBudget(180) }}><RotateCcw size={14} /> Reset</button></div>
            <label>Instrument<select value={selected} onChange={(e) => setSelected(e.target.value)}>{instruments.filter((item) => item.isRwa === 'YES').map((item) => <option key={item.symbol} value={item.symbol}>{item.baseCoin} / USDT perpetual</option>)}</select></label>
            ...
            <label className="thesis-label">Your thesis<textarea defaultValue="The catalyst resolves in my direction, but I need to know whether the position survives the path there." /></label>
      ...
      {tab === 'library' && <section className="content-page"><span className="step-label">LIBRARY</span><h1>Your rehearsals.</h1><p>Saved plans will appear here as Veltryn’s durable workspace arrives. The current slice keeps a local session receipt so you can inspect the core decision loop.</p><div className="empty-state"><BookOpen size={22} /><strong>No saved rehearsals yet</strong><span>Run a position test from Rehearse and save it to begin.</span><button className="outline-button" onClick={() => setTab('rehearse')}>Start a rehearsal</button></div></section>}
