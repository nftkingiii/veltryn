...
export function App() {
  ...
  const [liquidation, setLiquidation] = useState<{ status: 'loading' | 'ready' | 'unavailable' | 'error'; price?: number }>({ status: 'loading' })
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => getLiquidationPrice({ symbol: selected, direction, openAmount: quantity, openPrice: liveEntry }).then((result) => { if (!cancelled) setLiquidation({ status: result.status, price: result.liqPrice }) }).catch(() => { if (!cancelled) setLiquidation({ status: 'error' }) }), 350)
    return () => { cancelled = true; window.clearTimeout(timer) }
  }, [selected, direction, quantity, liveEntry])
